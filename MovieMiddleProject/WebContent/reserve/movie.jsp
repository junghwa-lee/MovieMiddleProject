 ...
 *    브라우저에 요청
       ...
       1) 서버연결
          req.open("POST", "URL", true/false) ==> true:비동기  false:동기화
 */
$(function(){
	$('#keyword').keyup(function(){
		let k=$('#keyword').val(); // 검색어 입력
		$('#movie-table > tbody > tr').hide();
		let temp=$('#movie-table > tbody > tr > td:nth-child(2n+2):contains("'+k+'")');
		$(temp).parent().show(); // temp = td이므로 tr부터 출력하도록 parent 설정
	});
	$('.movie').click(function(){
		...
		$('#mno').val(no); // reserve.jsp -> input에 값 채우기
		$.ajax({
			type:'post',
			url:'../reserve/theater.do',
			data:{"no":no},
			success:function(result)  // 실행결과받기
			{
